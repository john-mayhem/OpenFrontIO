...
- **Map picker: size shown on every card + sort-by-size**: land-tile-count badge on each card (bottom-left) and full pixel dimensions in the hover tooltip, sourced from `manifest.map.{width,height,num_land_tiles}` (every map has this, generated by the map tool — unlike the optional `info`/`designers` fields from #5180). New `src/client/components/map/MapSizes.ts` caches per-map size fetches (needed because `FetchGameMapLoader.manifest()` re-fetches on every call, doesn't memoize). A persistent "Sort by size" toggle sits next to the tab bar (Featured/All/Favorites/search) — lazily loads all 118 maps' sizes on first use, then sorts descending by land tiles. Known nuance: in the "All" tab it sorts *within* each category, not flattened across all of them — revisit if that's not what's wanted. Commit `0d228e7`.
